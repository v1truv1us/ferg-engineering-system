...
export class TaskExecutor {
    ...
    private async executeWithRetry(task: Task): Promise<TaskResult> {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            if (attempt < maxAttempts) {
                this.log(
                    `Task ${task.id} failed (attempt ${attempt}/${maxAttempts}): ${result.stderr || result.error}`,
                );
            }
        }
    ...
    private async executeAgentTask(
        ...
        if (!this.agentCoordinator) {
            throw new Error(
                `Agent coordinator not set. Cannot execute agent task: ${task.id}`,
            );
        }
        ...
        try {
            ...
            const agentResult =
                await this.agentCoordinator.executeTask(coordinatorTask);
            ...
            const result: TaskResult = {
                ...
                exitCode:
                    agentResult.status === AgentTaskStatus.COMPLETED ? 0 : 1,
            ...
            this.taskResults.set(task.id, result);
            this.log(
                `Agent task ${task.id} completed with status: ${result.status}`,
            );
    ...
    public async executeAgentTasks(
        ...
        this.log(
            `Executing ${tasks.length} agent tasks with strategy: ${strategy.type}`,
        );

        try {
            // Execute agent tasks using coordinator
            const agentResults = await this.agentCoordinator.executeTasks(
                tasks,
                strategy,
            );

            // Convert to task results
            const taskResults: TaskResult[] = agentResults.map(
                (agentResult) => ({
                    id: agentResult.id,
                    status: this.convertAgentStatus(agentResult.status),
                    exitCode:
            ...
            this.log(
                `Agent task execution failed: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
            throw error;
        }
    }
