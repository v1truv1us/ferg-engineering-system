...
As part of the architectural planning for the AI Engineering System, we evaluated two primary execution models for agent swarms and long-running tasks:
...
We have decided to **stick with Local Execution** for the current release.
...
While we are choosing local execution for now, the Cloud/SST architecture remains a valid candidate for future iterations. We may revisit this decision if:
...
*   **Positive**: Faster release cycle; easier installation for users (no AWS credentials needed); lower operating costs.
