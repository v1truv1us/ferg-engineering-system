...
# Import single agent with error handling
import_agent() {
  local source_path="$1"
...
update_documentation() {
  ...
  if [ ! -f "$agents_md" ]; then
    log_warning "AGENTS.md not found, skipping documentation update"
    return 0
  fi

  # Check if section already exists
  if grep -q "Imported CodeFlow Agents" "$agents_md"; then
    log_info "CodeFlow agents section already exists in documentation"
    return 0
  fi
