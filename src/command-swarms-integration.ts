...
export class CommandSwarmsIntegration {
  ...
  private initializeCommandMappings(): void {
    const mappings: Record<string, CommandSwarmMapping> = {
      ...
      review: {
        ...
        capabilities: ['code-quality', 'security', 'performance', 'frontend'],
      ...
      create: {
        command: 'create',
        description: 'AI-assisted creation of agents, commands, skills, tools',
  ...
  async executeCommand(
    ...
    try {
      ...
      if (options.fallbackToDirect) {
        console.warn(`Swarm execution failed for ${command}, falling back to direct execution:`, error);
        return this.executeDirectCommand(command, args);
      }
      throw error;
    }
   ...
   */
  private async executeDirectCommand(command: string, args: string[]): Promise<CommandResult> {
...
export function getCommandSwarmsIntegration(swarmsClient?: SwarmsClient): CommandSwarmsIntegration {
