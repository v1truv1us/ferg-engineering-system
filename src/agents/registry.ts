...
export class AgentRegistry {
    ...
    async loadFromDirectory(dir: string): Promise<void> {
        try {
            ...
            throw new Error(
                `Failed to load agents from directory ${dir}: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
    ...
    private parseFrontmatter(frontmatter: string): Record<string, any> {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (keyValueMatch) {
                ...
                if (currentKey) {
                    if (nestedObject) {
                        nestedObject[currentKey] = this.parseValue(
                            currentValue.trim(),
                        );
                    } else {
                        result[currentKey] = this.parseValue(
                            currentValue.trim(),
                        );
                    }
                }
