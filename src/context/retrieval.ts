...
export class ContextRetriever {
     ...
     */
    private async inferContextFromConversation(
        message: string,
        response: string,
    ): Promise<void> {
        if (!this.config.enableAutoInference) return;
     ...
     */
    private async inferContextFromCode(
        filePath: string,
        changes: string,
    ): Promise<void> {
        if (!this.config.enableAutoInference) return;
        ...
        for (const trigger of triggers) {
            switch (trigger.type) {
                ...
                case "file_open": {
                    ...
                    const fileSemanticResults =
                        await this.vectorManager.semanticSearch(
                            openedFilePath,
                            {
                                limit: 5,
                                minScore: 0.3,
                    ...
                    const traditionalMemories =
                        this.memoryManager.searchMemories(openedFilePath, {
                            minConfidence: 0.6,
                        });

                    memories.push(
                        ...fileSemanticResults.map((r) => r.memory),
                        ...traditionalMemories,
                    );
                    break;
                }
                ...
                case "command": {
                    ...
                    const semanticResults =
                        await this.vectorManager.semanticSearch(commandName, {
                            limit: 3,
                            minScore: 0.4,
                            memoryType: "procedural",
                        });
                    ...
                    // Traditional search
                    const commandMemories = this.memoryManager.searchMemories(
                        commandName,
                        {
                            minConfidence: 0.5,
                        },
                    ...
                    memories.push(
                        ...semanticResults.map((r) => r.memory),
                        ...commandMemories,
                    );
                    break;
                }
                ...
                case "query": {
                    ...
                    const semanticQueryResults =
                        await this.vectorManager.semanticSearch(userQuery, {
                            limit: 5,
                            minScore: 0.35,
                            memoryType: "procedural",
                        });

                    const traditionalQueryMemories =
                        this.memoryManager.searchMemories(userQuery, {
                            minConfidence: 0.5,
                        });

                    memories.push(
                        ...semanticQueryResults.map((r) => r.memory),
                        ...traditionalQueryMemories,
                    );
                    break;
                }
                ...
                case "file_edit": {
                    ...
                    await this.inferContextFromCode(
                        editedFilePath,
                        codeChanges,
                    );
                    ...
                    const context = {
                        ...
                        currentTask:
                            this.sessionManager.getContext<string>(
                                "currentTask",
                            ),
                        sessionType:
                            this.sessionManager.getSession()?.metadata.mode,
                    };

                    // Rank memories by relevance
                    const rankedMemories = ContextRanker.rankByRelevance(
                        allMemories,
                        context,
                    );
                ...
                case "task": {
                    ...
                    const taskMemories = this.memoryManager.searchMemories(
                        taskType,
                        {
                            tags: ["task"],
                            minConfidence: 0.5,
     ...
     */
    async getContextSummary(maxMemories = 5): Promise<string> {
        const session = this.sessionManager.getSession();
        const memories = this.memoryManager
            .getAllMemories()
            .slice(0, maxMemories);
        ...
        if (memories.length > 0) {
            ...
            for (const mem of memories) {
                lines.push(
                    `- [${mem.type}] ${mem.content.substring(0, 100)}...`,
                );
            }
            lines.push("");
        }
    ...
    estimateContextSize(context: AssembledContext): {
        ...
        return {
            ...
            memories: context.memories.reduce(
