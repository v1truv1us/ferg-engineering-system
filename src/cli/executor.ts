...
import { createCommandEnvelope, loadConfig } from "../context/types.js";
...
export class ExecutorCLI {
    ...
    private setupCommands(): void {
        ...
        this.program
            ...
            .option(
                "-c, --continue-on-error",
                "Continue execution after task failures",
            )
        ...
        this.program
            ...
            .option(
                "-s, --scope <scope>",
                "Plan scope (architecture|implementation|review|full)",
                "full",
            )
        ...
        this.program
            ...
            .option(
                "-t, --type <type>",
                "Review type (full|incremental|security|performance|frontend)",
                "full",
            )
            .option(
                "-s, --severity <severity>",
            ...
            .option(
                "-f, --focus <focus>",
                "Focused review (security|performance|frontend|general)",
            )
            .option(
                "-o, --output <file>",
                "Output report file",
        ...
        this.program
            ...
            .option(
                "-s, --scope <scope>",
                "Research scope (codebase|documentation|all)",
                "all",
            )
            .option(
                "-d, --depth <depth>",
        ...
        try {
            const report = await this.runWithEnvelope(
                ...
                async () => {
                    ...
                    console.log(
                        `📋 Plan: ${plan.metadata.name} v${plan.metadata.version}`,
                    );
                    console.log(
                    ...
                    if (plan.qualityGates && plan.qualityGates.length > 0) {
                        ...
                        qualityGateResults =
                            await gateRunner.executeQualityGates(
                                plan.qualityGates,
                            );
                    }
    ...
    private async executeGatesCommand(options: any): Promise<void> {
        ...
        try {
            const results = await this.runWithEnvelope(
                ...
                (r) =>
                    `Quality gates: ${r.filter((x) => x.passed).length} passed, ${r.filter((x) => !x.passed).length} failed`,
            );
        ...
        try {
            await this.runWithEnvelope(
                ...
                async () => {
                    ...
                    console.log(`🔧 Tasks: ${plan.tasks.length}`);
                    console.log(
                        `✅ Quality Gates: ${plan.qualityGates?.length || 0}`,
                    );
    ...
    private displayGateResults(results: QualityGateResult[]): void {
        ...
        for (const result of results) {
            ...
            console.log(
                `${status} ${result.gateId}: ${result.message} (${result.duration}ms)`,
            );
        }
    ...
    private async generatePlanCommand(
        ...
        try {
            await this.runWithEnvelope(
                ...
                (r: any) =>
                    `Generated plan '${r.plan?.name ?? "unknown"}' (confidence ${r.confidence})`,
            );
        } catch (error) {
            console.error(
    ...
    private async codeReviewCommand(
        ...
        try {
            await this.runWithEnvelope(
                ...
                async () => {
                    console.log(
                        `🔍 Starting code review for: ${files.join(", ")}`,
                    );
                    ...
                    let result: CodeReviewOutput;
                    if (options.focus) {
                        result =
                            await this.codeReviewExecutor.executeFocusedReview(
                                input,
                                options.focus,
                            );
                        ...
                        result =
                            await this.codeReviewExecutor.executeCodeReview(
                                input,
                            );
                    ...
                    if (options.verbose) {
                        ...
                        result.findings.forEach((finding, i) => {
                            console.log(
                                `  ${i + 1}. ${finding.file}:${finding.line} - ${finding.message}`,
                            );
                ...
                (r: any) =>
                    `Code review completed: ${r.findings?.length ?? 0} findings, score ${r.overallScore}/100`,
            );
        } catch (error) {
            console.error(
            ...
            const status = await this.runWithEnvelope(
                ...
                async () => {
                    ...
                    if (progress) {
                        ...
                        console.log(
                            `  Progress: ${progress.percentageComplete.toFixed(1)}%`,
                        );
                    }
                    ...
                    if (metrics && metrics.size > 0) {
                        ...
                        metrics.forEach((metric, agentType) => {
                            ...
                            console.log(
                                `    Avg Time: ${metric.averageExecutionTime.toFixed(0)}ms`,
                            );
                            console.log(
                                `    Confidence: ${metric.averageConfidence.toFixed(2)}`,
                            );
                        });
                    }
    ...
    private async executeResearchCommand(
        ...
        try {
            await this.runWithEnvelope(
                ...
                async () => {
                    console.log(
                        `🔍 Starting research: ${query || "(interactive mode)"}`,
                    );

                    // Parse scope and depth from options
                    const scope =
                        options.scope === "codebase"
                            ? ResearchScope.CODEBASE
                            : options.scope === "documentation"
                    ...
                    orchestrator.on(
                        ...
                        (progress: ResearchProgress) => {
                            ...
                            console.log(
                                `📊 Progress: ${progress.currentPhase} - ${percentage}% (${progress.completedSteps}/${progress.totalSteps})`,
                            );
                        },
                    );
                    ...
                    orchestrator.on("error", (error: ResearchError) => {
                        console.error(
                            `❌ Research error in ${error.phase}: ${error.error}`,
                        );
                    });
                    ...
                    console.log(`🔍 Scope: ${result.metadata.scope}`);
                    console.log(`⚡ Depth: ${result.metadata.depth}`);
                    console.log(
                        `⏱️  Duration: ${result.metrics?.totalDuration ?? result.executionTime}ms`,
                    );
                    ...
                    if (result.findings.length > 0) {
                        ...
                        result.findings.forEach((finding, i) => {
                            console.log(
                                `  ${i + 1}. ${finding.title} (${finding.impact})`,
                            );
                            if (
                                finding.evidence &&
                                ...
                                console.log(
                                    `     Evidence: ${finding.evidence.length} sources`,
                                );
                            }
                        });
                    }
                ...
                (r: any) =>
                    `Research completed: ${r.findings?.length ?? 0} findings, ${r.recommendations?.length ?? 0} recommendations`,
            );
        } catch (error) {
            console.error(
