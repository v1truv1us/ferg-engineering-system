...
export class CodebaseAnalyzer implements AnalysisAgent {
    ...
    async analyze(
        ...
        try {
            ...
            throw new Error(
                `Codebase analyzer failed: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
        ...
        for (const pattern of patterns) {
            ...
            while ((match = pattern.regex.exec(content)) !== null) {
                ...
                evidence.push({
                    ...
                    file: file.path,
                    line: lineNumber,
                    confidence: this.assessEvidenceConfidence(
                        match[0],
                        pattern.type,
                    ),
    ...
    private async generateInsights(
        ...
        insights.push(
            ...this.generateArchitecturalInsights(evidence, discoveryResults),
        );
    ...
    private generatePatternInsights(
        evidenceByType: Record<string, Evidence[]>,
    ): Insight[] {
        const insights: Insight[] = [];
    ...
    private generateFileInsights(
        evidenceByFile: Record<string, Evidence[]>,
    ): Insight[] {
        const insights: Insight[] = [];
        ...
        for (const [file, items] of Object.entries(evidenceByFile)) {
            ...
            const debtItems = items.filter(
                (e) =>
                    e.content.includes("TODO") || e.content.includes("FIXME"),
            );
    ...
    private analyzeImportSources(imports: Evidence[]): {
        internal: number;
        external: number;
    } {
        let internal = 0;
    ...
    private async identifyRelationships(
        ...
        for (let i = 0; i < insights.length; i++) {
            for (let j = i + 1; j < insights.length; j++) {
                ...
                const sharedEvidence = insight1.evidence.filter((e) =>
                    insight2.evidence.includes(e),
                );
                if (sharedEvidence.length > 0) {
                    relationships.push({
                        ...
                        strength:
                            ...
                            Math.max(
                                insight1.evidence.length,
                                insight2.evidence.length,
                            ),
                ...
                if (
                    ...
                    insight1.category !== "pattern-analysis"
                ) {
                    relationships.push({
    ...
    private calculateOverallConfidence(
        ...
        const insightConfidence =
            insights.reduce((sum, insight) => {
                const confidenceValue = this.confidenceToNumber(
                    insight.confidence,
                );
...
export class ResearchAnalyzer implements AnalysisAgent {
    ...
    async analyze(
        ...
        try {
            ...
            const patternEvidence =
                await this.analyzePatterns(discoveryResults);
            evidence.push(...patternEvidence);

            // 4. Generate insights from documentation
            const insights = await this.generateDocumentationInsights(
                evidence,
                discoveryResults,
            );

            // 5. Identify relationships
            const relationships = await this.identifyDocumentationRelationships(
                insights,
                evidence,
            );
            ...
            throw new Error(
                `Research analyzer failed: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
    ...
    private async extractDocumentationEvidence(
        docs: DocReference[],
    ): Promise<Evidence[]> {
        const evidence: Evidence[] = [];

        for (const doc of docs.slice(0, 15)) {
            // Limit to top 15 docs
            try {
                const content = await readFile(doc.path, "utf-8");
                const docEvidence = this.analyzeDocumentationForEvidence(
                    doc,
                    content,
                );
                ...
                evidence.push({
                    ...
                    relevance:
                        pattern.matches.length > 0
                            ? Math.max(
                                  ...pattern.matches.map((m) => m.relevance),
                              )
                            : 0.5,
                });
            }
    ...
    private async generateDocumentationInsights(
        ...
        insights.push(
            ...this.generateCompletenessInsights(evidence, discoveryResults),
        );
    ...
    private generateDocumentationQualityInsights(
        ...
        for (const [file, items] of Object.entries(evidenceByFile)) {
            ...
            const links = items.filter(
                (e) => e.content.includes("[") && e.content.includes("]("),
            );
    ...
    private async identifyDocumentationRelationships(
        insights: Insight[],
        evidence: Evidence[],
    ): Promise<Relationship[]> {
        const relationships: Relationship[] = [];
...
export class AnalysisHandler {
    ...
    getAnalysisMetrics(results: {
        ...
        const executionTime =
            codebaseAnalysis.executionTime + researchAnalysis.executionTime;
