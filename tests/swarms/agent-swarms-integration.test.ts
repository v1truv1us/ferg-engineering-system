...
describe("AgentSwarmsIntegration", () => {
    ...
    describe("Agent Registry", () => {
        ...
        it("should have all AI & Machine Learning agents", () => {
            const aiAgents = ["ai-engineer", "ml-engineer"];
    ...
    describe("Capability-Based Search", () => {
        it("should find agents with architecture capability", () => {
            const agents = integration.findAgentsByCapabilities([
                "architecture",
            ]);
        ...
        it("should filter by minimum confidence", () => {
            ...
            const highConfAgents = integration.findAgentsByCapabilities(
                ["architecture"],
                0.9,
            );

            // Lower confidence threshold - should return more agents
            const lowConfAgents = integration.findAgentsByCapabilities(
                ["architecture"],
                0.5,
            );

            expect(highConfAgents.length).toBeLessThanOrEqual(
                lowConfAgents.length,
            );
        });
