...
export class CodebaseLocator implements DiscoveryAgent {
    ...
    async discover(query: ResearchQuery): Promise<DiscoveryResult> {
        ...
        try {
            ...
            throw new Error(
                `Codebase locator failed: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
    ...
    private async findFiles(
        ...
        const uniqueFiles = Array.from(
            new Map(
                allFiles.map((f) => [f.path, f] as [string, FileReference]),
            ).values(),
        );
        ...
        return files
            .map((file) => {
                ...
                const daysSinceModified =
                    (Date.now() - (file.lastModified?.getTime() || 0)) /
                    (1000 * 60 * 60 * 24);
    ...
    private calculateConfidence(
        ...
        const avgRelevance =
            files.reduce((sum, file) => sum + file.relevance, 0) / files.length;
...
export class ResearchLocator implements DiscoveryAgent {
    ...
    async discover(query: ResearchQuery): Promise<DiscoveryResult> {
        ...
        try {
            ...
            throw new Error(
                `Research locator failed: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
    ...
    private async findDocumentation(
        ...
        return Array.from(
            new Map(
                allDocs.map((d) => [d.path, d] as [string, DocReference]),
            ).values(),
        );
    }
    ...
    private meetsDocConstraints(
        ...
        if (constraints.dateRange && doc.lastModified) {
            if (
                constraints.dateRange.from &&
                doc.lastModified < constraints.dateRange.from
            ) {
                return false;
            }
            if (
                constraints.dateRange.to &&
                doc.lastModified > constraints.dateRange.to
            ) {
                return false;
            }
        }
    ...
    private calculateConfidence(
        ...
        const avgRelevance =
            docs.reduce((sum, doc) => sum + doc.relevance, 0) / docs.length;
...
export class PatternFinder implements DiscoveryAgent {
    ...
    async discover(query: ResearchQuery): Promise<DiscoveryResult> {
        ...
        try {
            ...
            const matches = await this.findSimilarCode(
                targetPatterns,
                query.constraints,
            );
            ...
            throw new Error(
                `Pattern finder failed: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
...
export class DiscoveryHandler {
    ...
    async discover(query: ResearchQuery): Promise<DiscoveryResult[]> {
        ...
        try {
            ...
            const successfulResults = results
                ...
                .map(
                    (r) => (r as PromiseFulfilledResult<DiscoveryResult>).value,
                );
            ...
            throw new Error(
                `Discovery handler failed: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
    ...
    private deduplicateResults(results: DiscoveryResult[]): DiscoveryResult[] {
        ...
        for (const result of results) {
            const uniqueFiles = result.files.filter(
                (f) => !seenFiles.has(f.path),
            );
            const uniqueDocs = result.documentation.filter(
                (d) => !seenDocs.has(d.path),
            );
            const uniquePatterns = result.patterns.filter(
                (p) => !seenPatterns.has(p.pattern),
            );
