...
export class PlanGenerator {
    ...
    private aggregatePlanResults(
        ...
        const planDescription = this.generatePlanDescription(
            input,
            successfulResults,
        );
    ...
    private generateReasoning(
        ...
        const reasonings = successfulResults
            .map((r) => r.output?.reasoning || "")
            .filter(Boolean);

        return (
            `Generated comprehensive plan based on: ${input.description}. ` +
        ...
        for (const result of results) {
            if (
                ...
                if (result.output?.result?.issues) {
                    ...
                    if (
                        result.output.result.issues.some(
    ...
    private generatePlanDescription(
        ...
        return (
            `Implementation plan for: ${input.description}. ` +
