...
export class ExecutorBridge {
  ...
  selectExecutionMode(task: AgentTask): ExecutionMode {
    ...
    const hasFileOperations = task.input?.context?.files ||
  ...
  async execute(task: AgentTask): Promise<AgentOutput> {
    ...
    return Promise.race([
      ...
      new Promise<AgentOutput>((_, reject) =>
        setTimeout(() => reject(new Error(`Agent ${task.type} timed out after ${timeout}ms`)), timeout)
      )
    ]);
  }
    ...
    try {
      ...
      switch (task.type) {
        ...
        case AgentType.CODE_REVIEWER:
          ...
          break;
        default:
          result = { operation: 'generic', data: 'Local execution completed' };
      }
  ...
  mapToSubagentType(type: AgentType): string {
    const mapping: Record<AgentType, string> = {
      ...
      [AgentType.PERFORMANCE_ENGINEER]: 'quality-testing/performance_engineer',
      [AgentType.BACKEND_ARCHITECT]: 'development/backend_architect',
      [AgentType.FRONTEND_REVIEWER]: 'design-ux/frontend-reviewer',
      [AgentType.FULL_STACK_DEVELOPER]: 'development/full_stack_developer',
      [AgentType.API_BUILDER_ENHANCED]: 'development/api_builder_enhanced',
   ...
   */
  async buildEnhancedPrompt(agent: AgentDefinition, task: AgentTask): Promise<string> {
...
- Timeout: ${task.timeout || 'default'}`;
  ...
  private buildExpertPersona(agent: AgentDefinition): string {
    ...
    const companies = ['Google', 'Stripe', 'Netflix', 'Meta', 'Amazon', 'Microsoft'];
    ...
    try {
      ...
      switch (operation.operation) {
        ...
        case 'grep':
          ...
          for (const file of grepFiles.slice(0, 10)) { // Limit to 10 files
            try {
