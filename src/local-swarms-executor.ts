 ...
 */
export class LocalSwarmsExecutor extends SwarmsClient {
    ...
    // Implement request method for interface compliance (not used for local execution)
    public async request<T>(
        endpoint: string,
        options: RequestInit = {},
    ): Promise<T> {
        throw new Error(
            "LocalSwarmsExecutor does not support HTTP requests. Use executeTask instead.",
        );
    }
 ...
 */
export function createSwarmsClient(
    options: {
...
export async function checkTypeScriptSwarmsAvailable(): Promise<boolean> {
    try {
        const {
            checkTypeScriptSwarmsAvailable,
        } = require("./typescript-swarms-executor.js");
