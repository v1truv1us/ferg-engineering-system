...
import { SwarmConfig, Swarm } from '../../src/swarms-client.js';
...
describe('LocalSwarmsExecutor', () => {
  ...
  describe('Swarm Creation', () => {
    ...
    it('should support all swarm types', async () => {
      const swarmTypes: Array<'MultiAgentRouter' | 'SequentialWorkflow' | 'AgentRearrange' | 'ConcurrentWorkflow'> = [
        'MultiAgentRouter',
        'SequentialWorkflow', 
        'AgentRearrange',
        'ConcurrentWorkflow'
  ...
  describe('Swarm Management', () => {
    ...
    it('should throw error for non-existent swarm', async () => {
      await expect(executor.getSwarm('non-existent-id')).rejects.toThrow('Swarm not found');
    });
    ...
    it('should delete a swarm', async () => {
      ...
      await expect(executor.getSwarm(swarm.id)).rejects.toThrow('Swarm not found');
    });
  });
...
describe('LocalSwarmsExecutor - Task Execution', () => {
  ...
  it('should return task result structure', async () => {
    const result = await executor.runTask(swarmId, 'Test task', { timeout: 5000 });
