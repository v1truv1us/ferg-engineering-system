...
export class PlanParser {
    ...
    private parseMetadata(metadata: any): PlanMetadata {
        ...
        if (metadata.version && !/^\d+\.\d+\.\d+/.test(metadata.version)) {
            this.warnings.push(
                `metadata.version "${metadata.version}" should follow semantic versioning (x.y.z)`,
            );
        }
    ...
    private parseTask(taskData: any, index: number): Task | null {
        ...
        if (taskData.retry) {
            if (
                !taskData.retry.maxAttempts ||
                typeof taskData.retry.maxAttempts !== "number" ||
                taskData.retry.maxAttempts < 1
            ) {
                this.errors.push({
                    type: ValidationErrorType.RANGE,
            ...
            if (
                !taskData.retry.delay ||
                typeof taskData.retry.delay !== "number" ||
                taskData.retry.delay < 0
            ) {
                this.errors.push({
                    type: ValidationErrorType.RANGE,
    ...
    private parseAgentTask(taskData: any, index: number): AgentTask | null {
        ...
        if (
            !taskData.type ||
            !Object.values(AgentTaskType).includes(taskData.type)
        ) {
            this.errors.push({
                type: ValidationErrorType.TYPE,
        ...
        if (
            !taskData.strategy ||
            !Object.values(ExecutionStrategy).includes(taskData.strategy)
        ) {
            this.errors.push({
                type: ValidationErrorType.REQUIRED,
        ...
        if (
            !agentInput.type ||
            !Object.values(AgentTaskType).includes(agentInput.type)
        ) {
            this.errors.push({
                type: ValidationErrorType.REQUIRED,
            ...
            if (this.isAgentTask(task) && task.dependsOn) {
                for (const depId of task.dependsOn) {
                    ...
                    if (depTask && !this.isAgentTask(depTask)) {
                        this.warnings.push(
                            `Agent task "${task.id}" depends on shell task "${depId}". Consider using agent tasks for consistency.`,
                        );
                    }
                }
            }
    ...
    public validateAgentTaskConfiguration(plan: Plan): {
        ...
        if (agentTasks.length > 0) {
            warnings.push(
                `Plan contains ${agentTasks.length} agent task(s). Agent coordinator must be configured.`,
            );
        }
        ...
        for (const task of agentTasks) {
            if (!task.timeout || task.timeout < 5000) {
                warnings.push(
                    `Agent task "${task.id}" has short or missing timeout. Consider setting at least 5 seconds.`,
                );
            }
        }
        ...
        for (const task of agentTasks) {
            if (!task.retry) {
                warnings.push(
                    `Agent task "${task.id}" has no retry configuration. Consider adding retry logic for reliability.`,
                );
            }
        }
