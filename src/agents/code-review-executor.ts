...
export class CodeReviewExecutor {
    ...
    public async executeCodeReview(
        ...
        try {
            ...
            throw new Error(
                `Code review execution failed: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
    ...
    private createReviewTasks(input: CodeReviewInput): AgentTask[] {
        ...
        if (this.shouldIncludeSecurityReview(input)) {
            tasks.push({
                ...
                description:
                    "Review code for security vulnerabilities and issues",
                input: {
                    type: AgentType.SECURITY_SCANNER,
                    context: {
                        ...baseContext,
        ...
        if (this.shouldIncludeFrontendReview(input)) {
            tasks.push({
                ...
                description:
                    "Review frontend code for UX, accessibility, and best practices",
                input: {
                    type: AgentType.FRONTEND_REVIEWER,
                    context: {
                        ...baseContext,
    ...
    private generateRecommendations(
        ...
        if (criticalFindings.length > 0) {
            recommendations.push(
                `Address ${criticalFindings.length} critical security/stability issues immediately`,
            );
        }

        if (highFindings.length > 0) {
            recommendations.push(
                `Prioritize fixing ${highFindings.length} high-priority issues`,
            );
        }
        ...
        if (findings.length > 10) {
            recommendations.push(
                "Consider breaking down this review into smaller, focused reviews",
            );
        }
        ...
        if (securityFindings.length > 0) {
            recommendations.push(
                "Schedule a dedicated security review to address all security concerns",
            );
        }
        ...
        if (performanceFindings.length > 0) {
            recommendations.push(
                "Consider performance testing to validate optimization improvements",
            );
        }
    ...
    private shouldIncludePerformanceReview(input: CodeReviewInput): boolean {
        return (
            input.reviewType === "full" || input.reviewType === "performance"
        );
    }
    ...
    private shouldIncludeFrontendReview(input: CodeReviewInput): boolean {
        return (
            input.reviewType === "full" || this.hasFrontendFiles(input.files)
        );
    }
