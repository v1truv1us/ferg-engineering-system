...
describe('ExecutorBridge', () => {
  ...
  describe('Execution Mode Selection', () => {
    it('should use Task tool for complex reasoning tasks', async () => {
      const task = {
        ...
        input: {
          type: AgentType.ARCHITECT_ADVISOR,
          context: { description: 'Analyze microservices architecture' }
        },
    ...
    it('should use Task tool for agents requiring LLM reasoning', async () => {
      ...
      for (const agentType of llmRequiredAgents) {
        ...
        const mode = bridge.selectExecutionMode(task);
        expect(mode).toBe('task-tool', `Expected Task tool for ${agentType}`);
      }
    });
    ...
    it('should use local execution for data processing agents', async () => {
      ...
      for (const agentType of dataProcessingAgents) {
        const task = {
          ...
          type: agentType,
          name: 'Test',
          input: { type: agentType, context: { operation: 'analyze' } },
          strategy: 'sequential' as const
        };
  ...
  describe('Task Tool Integration', () => {
    it('should map AgentType to correct subagent_type', () => {
      expect(bridge.mapToSubagentType(AgentType.CODE_REVIEWER))
        .toBe('quality-testing/code_reviewer');
      expect(bridge.mapToSubagentType(AgentType.ARCHITECT_ADVISOR))
        .toBe('development/system_architect');
      expect(bridge.mapToSubagentType(AgentType.SECURITY_SCANNER))
        .toBe('quality-testing/security_scanner');
      expect(bridge.mapToSubagentType(AgentType.AI_ENGINEER))
        .toBe('ai-innovation/ai_engineer');
    });
    ...
    it('should handle agents with different expertise levels', async () => {
      ...
      const task = {
        ...
        input: { type: AgentType.ARCHITECT_ADVISOR, context: { system: 'e-commerce' } },
        strategy: 'sequential' as const
      };
  ...
  describe('Error Handling', () => {
    it('should handle Task tool failures gracefully', async () => {
      ...
      await expect(bridge.execute(failingTask)).rejects.toThrow('timed out after 1ms');
    });
