...
describe("QualityGateRunner", () => {
    ...
    describe("Default Gates", () => {
        it("should provide default quality gates configuration", () => {
            ...
            const lintGate = defaultGates.find(
                (g) => g.type === QualityGateType.LINT,
            );
            const integrationGate = defaultGates.find(
                (g) => g.type === QualityGateType.INTEGRATION,
            );
    ...
    describe("Configuration", () => {
        it("should use custom timeout and command from config", async () => {
            ...
            expect(result.passed).toBe(true);
            expect(result.details?.taskResult?.stdout).toContain(
                "Custom command",
            );
        });
    });
