...
export class QualityGateRunner {
    ...
    public async executeQualityGates(
        ...
        for (const gate of sortedGates) {
            ...
            if (gate.required && !result.passed) {
                this.log(
                    `Stopping execution due to required quality gate failure: ${gate.id}`,
                );
                break;
            }
        }
    ...
    public async executeQualityGate(
        ...
        try {
            ...
            this.log(
                `Quality gate ${gate.id} ${passed ? "passed" : "failed"} in ${duration}ms`,
            );
            ...
            this.log(
                `Quality gate ${gate.id} failed with error: ${errorMessage}`,
            );
    ...
    public static createGatesFromTasks(tasks: Task[]): QualityGateConfig[] {
        ...
        for (const task of tasks) {
            if (QualityGateRunner.isQualityGateTask(task)) {
                const gateType = QualityGateRunner.mapTaskTypeToGateType(
                    task.type,
                );
    ...
    private static mapTaskTypeToGateType(taskType: TaskType): QualityGateType {
        const mapping: Partial<Record<TaskType, QualityGateType>> = {
    ...
    private evaluateGateResult(
        gate: QualityGateConfig,
        taskResult: any, // Using any to support both TaskResult and QualityGateResult
    ): boolean {
