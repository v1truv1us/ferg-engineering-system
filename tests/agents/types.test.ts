...
describe('AgentType Enum Completeness', () => {
  const expectedAgents = [
    ...
    // Development & Coding
    'frontend-reviewer', 'full-stack-developer', 'api-builder-enhanced',
    'database-optimizer', 'java-pro',
    // Quality & Testing
  ...
  it('should have all 24 agents defined in AgentType enum', () => {
    ...
    for (const agent of expectedAgents) {
      const found = enumValues.some(v => v === agent || v.replace(/_/g, '-') === agent);
  ...
  it('should have consistent naming (kebab-case)', () => {
    for (const value of Object.values(AgentType)) {
      expect(value).toMatch(/^[a-z]+(-[a-z]+)*$/, `Invalid format: ${value}`);
    }
  });
