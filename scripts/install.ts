...
function install(targetDir: string, silent = false): void {
    ...
    if (fs.existsSync(commandsSrc)) {
        ...
        if (!silent) console.log(`  ✓ command/${NAMESPACE_PREFIX}/ (${commandCount} commands)`)
    }
...
function main(): void {
    ...
    if (isPostInstall) {
        ...
        if (!configPath) {
            console.error('❌ Error: opencode.jsonc not found in current directory or parent directories')
            console.error('   Please run this script from a project containing opencode.jsonc')
            process.exit(1)
        }
