...
describe("Enhanced Task Executor", () => {
    ...
    describe("Agent Task Execution", () => {
        ...
        it("should execute multiple agent tasks", async () => {
            ...
            expect(
                results.every((r) => r.status === TaskStatus.COMPLETED),
            ).toBe(true);
...
tasks:
  ...
  `;
            ...
            expect(validation.warnings.some((w) => w.includes("timeout"))).toBe(
                true,
            );
            expect(validation.warnings.some((w) => w.includes("retry"))).toBe(
                true,
            );
        });
    });
...
tasks:
  ...
  `;
        ...
        expect(results.every((r) => r.status === TaskStatus.COMPLETED)).toBe(
            true,
        );
