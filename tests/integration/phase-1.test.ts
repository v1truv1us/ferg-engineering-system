...
describe("Execution Engine Integration", () => {
    ...
    beforeEach(() => {
        ...
        gateRunner = new QualityGateRunner({
            workingDirectory: testDir,
            verbose: false,
        });
    });
...
`;
            ...
            const gateResults = await gateRunner.executeQualityGates(
                plan.qualityGates!,
            );
            ...
            expect(setupResult?.startTime.getTime()).toBeLessThan(
                buildResult?.startTime.getTime(),
            );
            expect(buildResult?.startTime.getTime()).toBeLessThan(
...
`;
            ...
            const dependentResult =
                taskExecutor.getTaskResult("dependent-task");
            const independentResult =
                taskExecutor.getTaskResult("independent-task");
        ...
        it("should handle quality gate failures correctly", async () => {
            ...
            const passingResult = results.find(
                (r) => r.gateId === "passing-gate",
            );
            const failingResult = results.find(
                (r) => r.gateId === "failing-gate",
            );
...
`;
    ...
    describe("Configuration and Options", () => {
        it("should respect dry run mode", async () => {
            const dryExecutor = new TaskExecutor({
                dryRun: true,
                workingDirectory: testDir,
            });
            const dryRunner = new QualityGateRunner({
                dryRun: true,
                workingDirectory: testDir,
            });
...
`;
            ...
            const gateResults = await dryRunner.executeQualityGates(
                plan.qualityGates!,
            );
            ...
            expect(gateResults[0].details?.taskResult?.stdout).toContain(
                "[DRY RUN]",
            );
        });
    });
