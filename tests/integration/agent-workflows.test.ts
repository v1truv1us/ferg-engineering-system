...
describe("Agent Workflow Integration", () => {
    ...
    describe("Agent Coordination", () => {
        it("should coordinate multiple agents successfully", async () => {
            ...
            expect(
                results.every((r) => r.status === AgentTaskStatus.COMPLETED),
            ).toBe(true);
...
tasks:
  ...
  `;
            ...
            const agentTaskResult = results.find(
                (r) => r.id === "agent-review-task",
            );
...
tasks:
  ...
  `;
            ...
            const agentTask1Result = results.find(
                (r) => r.id === "agent-task-1",
            );
            const agentTask2Result = results.find(
                (r) => r.id === "agent-task-2",
            );
