...
export class AgentCommunicationHub extends EventEmitter {
   ...
   */
  async requestHandoff(request: Omit<HandoffRequest, 'id' | 'timestamp'>): Promise<string> {
  ...
  getAgentMessages(agent: AgentType, limit = 50): AgentMessage[] {
    return this.messageHistory
      .filter(msg => msg.to === agent || msg.to === 'broadcast' || msg.from === agent)
  ...
  getPendingHandoffs(agent: AgentType): HandoffRequest[] {
    return Array.from(this.pendingHandoffs.values())
      .filter(request => request.toAgent === agent);
  }

  private async createChannel(participants: AgentType[], topic: string): Promise<string> {
  ...
  private addMessageToChannels(message: AgentMessage): void {
    for (const channel of this.channels.values()) {
      if (channel.active &&
          (channel.participants.includes(message.from) ||
           (message.to !== 'broadcast' && channel.participants.includes(message.to as AgentType)))) {
    ...
    for (const session of this.activeSessions.values()) {
      if (session.participants.includes(message.from) ||
          (message.to !== 'broadcast' && session.participants.includes(message.to as AgentType))) {
  ...
  private setupEventHandlers(): void {
    ...
    this.on('collaboration_completed', (session: CollaborationSession) => {
      console.log(`✅ Collaboration completed: ${session.id} with outcome:`, session.outcome);
    });
  }
...
export function getAgentCommunicationHub(memoryManager?: MemoryManager): AgentCommunicationHub {
