...
export class SynthesisHandlerImpl implements SynthesisHandler {
    ...
    async synthesize(
        ...
        try {
            ...
            const allRelationships =
                this.collectAllRelationships(analysisResults);

            // 2. Generate synopsis
            const synopsis = this.generateSynopsis(
                query,
                allInsights,
                allEvidence,
            );
            ...
            const findings = this.generateDetailedFindings(
                allInsights,
                allEvidence,
            );
    ...
    private collectAllEvidence(analysisResults: AnalysisResult[]): Evidence[] {
        ...
        const uniqueEvidence = evidence.filter(
            (ev, index, self) =>
                index ===
                self.findIndex(
                    (e) => e.content === ev.content && e.file === ev.file,
                ),
        );
    ...
    private generateSynopsis(
        ...
        synopsis +=
            ". The analysis identified patterns in code structure, documentation quality, and architectural decisions that provide a comprehensive understanding of the current state.";
    ...
    private generateSummary(
        ...
        summary.push(
            `Found ${insights.length} insights across ${evidence.length} evidence points`,
        );
        ...
        // Impact summary
        const highImpactInsights = insights.filter((i) => i.impact === "high");
        const mediumImpactInsights = insights.filter(
            (i) => i.impact === "medium",
        );

        if (highImpactInsights.length > 0) {
            summary.push(
                `${highImpactInsights.length} high-impact findings require immediate attention`,
            );
        }

        if (mediumImpactInsights.length > 0) {
            summary.push(
                `${mediumImpactInsights.length} medium-impact findings should be addressed in the near term`,
            );
        }

        // Evidence quality summary
        const highConfidenceEvidence = evidence.filter(
            (e) => e.confidence === ConfidenceLevel.HIGH,
        );
        if (highConfidenceEvidence.length > 0) {
            summary.push(
                `${highConfidenceEvidence.length} high-confidence evidence points support the findings`,
            );
        }
        ...
        if (query.scope === ResearchScope.CODEBASE) {
            ...
            summary.push(
                `Analysis focused on ${codeEvidence.length} code elements across the codebase`,
            );
        } else if (query.scope === ResearchScope.DOCUMENTATION) {
            const docEvidence = evidence.filter(
                (e) => e.type === "documentation",
            );
            summary.push(
                `Analysis reviewed ${docEvidence.length} documentation elements`,
            );
        }
    ...
    private generateArchitectureInsights(
        ...
        const archInsights = insights.filter(
            (i) =>
        ...
        if (strongRelationships.length > 0) {
            architectureInsights.push({
                ...
                components:
                    this.extractComponentsFromRelationships(
                        strongRelationships,
                    ),
                impact: "medium",
                evidence: strongRelationships
                    .slice(0, 3)
    ...
    private mapInsightTypeToArchType(
        insightType: string,
    ): "pattern" | "decision" | "concern" | "recommendation" {
        switch (insightType) {
            case "pattern":
                return "pattern";
            case "decision":
    ...
    private generateRecommendations(
        ...
        const mediumImpactFindings = findings.filter(
            (f) => f.impact === "medium",
        );
        ...
        for (const finding of mediumImpactFindings.slice(0, 3)) {
            recommendations.push({
                ...
                rationale:
                    "This medium-impact finding should be addressed to improve overall quality",
                effort: this.estimateEffort(finding),
                impact: finding.impact,
                dependencies: [],
            });
        ...
        if (archInsights.length > 0) {
            recommendations.push({
                ...
                description:
                    "Consider implementing architectural improvements based on identified patterns",
                rationale:
                    "Analysis revealed architectural patterns that could be optimized for better maintainability",
                effort: "high",
                impact: "high",
                dependencies: [],
            });
    ...
    private generateRisks(
        ...
        if (debtFindings.length > 2) {
            risks.push({
                ...
                impact: "high",
                mitigation:
                    "Implement regular refactoring sprints and address technical debt items systematically",
    ...
    private generateOpenQuestions(
        ...
        if (insights.length === 0) {
            questions.push(
                "Why were no significant insights found? Is the query too broad or the scope too limited?",
            );
        }

        if (evidence.length < 10) {
            questions.push(
                "Is there additional evidence that could be collected to support more comprehensive analysis?",
            );
        }
        ...
        if (!categories.includes("architecture")) {
            questions.push(
                "What architectural patterns and decisions should be further investigated?",
            );
        }

        if (!categories.includes("performance")) {
            questions.push(
                "Are there performance considerations that should be analyzed?",
            );
        }
        ...
        if (query.scope === ResearchScope.CODEBASE) {
            questions.push(
                "How does the codebase structure align with industry best practices and standards?",
            );
        } else if (query.scope === ResearchScope.DOCUMENTATION) {
            questions.push(
                "How can the documentation be improved to better support development and maintenance?",
            );
        }

        // Generate forward-looking questions
        questions.push(
            "What steps should be taken to address the identified findings and risks?",
        );
        questions.push(
            "How can the research process be improved for future analyses?",
        );
    ...
    private calculateOverallConfidence(
        ...
        const insightScores = insights.map((i) =>
            this.confidenceToNumber(i.confidence),
        );
        const evidenceScores = evidence.map((e) =>
            this.confidenceToNumber(e.confidence),
        );
    ...
    private countUniqueFiles(evidence: Evidence[]): number {
        const files = new Set(
            evidence.filter((e) => e.file).map((e) => e.file),
        );
        return files.size;
    }
    ...
    async exportReport(
        ...
        const outputPath =
            options.outputPath ||
            `research-report-${Date.now()}.${options.format}`;
    ...
    private async exportToMarkdown(
        ...
        try {
            ...
            throw new Error(
                `Failed to export markdown report: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
    ...
    private async exportToJSON(
        ...
        try {
            ...
            throw new Error(
                `Failed to export JSON report: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
    ...
    private async exportToHTML(
        ...
        try {
            ...
            throw new Error(
                `Failed to export HTML report: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
